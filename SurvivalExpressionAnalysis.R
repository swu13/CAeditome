#!/usr/bin/R
#parameter1: Gene expression file, row: Gene, col:tumor samples with header
...
colnames(Outputdata)=c("Gene","Pkm","Pcox_discrete","HR_discrete","HR_lower_discrete","HR_upper_discrete","Pcox_continuous","HR_continuous","HR_lower_continuous","HR_upper_continuous")
#survival analysis
...
InformativeExpression=EachData[InformativeIndex]
InformativeStatus=SurvivalStatusNum[InformativeIndex]
InformativeTime=SurvialTime[InformativeIndex]
TranferExpression=ifelse(InformativeExpression > median(InformativeExpression),1,0)
survival_data <- data.frame(group = TranferExpression,status = InformativeStatus,time = InformativeTime,stringsAsFactors = F)
#KM
HighIndex=which(TranferExpression==1)
LowIndex=which(TranferExpression==0)
...
#COX--continue
survival_data <- data.frame(group = InformativeExpression,status = InformativeStatus,time = InformativeTime,stringsAsFactors = F)
if(length(which(is.na(survival_data$time)=="FALSE"))<2){
...
if((is.na(Outputdata[i,4]) && (length(LowIndexDead)==1 || length(HighIndexDead)==1) && (length(LowIndexDead)<length(HighIndexDead))) || (! is.na(Outputdata[i,4]) && Outputdata[i,4]>1)){
lengend_name=c("LowExpression","HighExpression")
