#!/usr/bin/R
#Input: 
#parameter1: expression file,Column:Samples; Row: RNA editing without header
...
print(ggboxplot(figure_data, x = "Group", y = "Editing",color = "Group", palette = c("#800000", "#000080"),add = "jitter")+border()+ylab("Gene Expression")+theme(legend.position="none",axis.text=element_text(face = "bold",hjust=0.5),axis.title.x=element_text(size=0),axis.title.y=element_text(face = "bold"))+stat_compare_means(aes(group=Group),label = "p.signif",label.y=0.9,label.x=1.5,face="bold",hide.ns=T, paired=F,method="t.test"))
dev.off()
}
}
